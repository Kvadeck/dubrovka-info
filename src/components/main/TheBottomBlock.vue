...
<template>
  <div>
    ...
    <Title class="text-[50px] color-white leading-[50px] lg:whitespace-nowrap lg:text-[80px] max-md:text-[48px] xl:text-[100px] lg:leading-[95px] max-md:leading-[48px]">
      Квартиры и пентхаусы<br>бизнес-класса
    </Title>
    <div v-if="activeBreakpoint === 'sm'" class="relative py-4">
      <ComfortPlans />
    </div>
    <div v-if="activeBreakpoint === 'sm'" class="relative max-w-[316px]">
      <ChooseFlatBtn />
    </div>
    <div v-if="activeBreakpoint !== 'lg'" class="relative py-7">
      <AdditionalLinks />
    </div>
  </div>
</template>
